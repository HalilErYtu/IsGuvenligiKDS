...
class PFPMC(_ab._periodicFrequentPatterns):
    ...
    def __init__(self, file_name, minsup, maxPer, sep):
        super().__init__(iFile=file_name, minSup=minsup, maxPer=maxPer, sep=sep)
        self._iFile = file_name
        self._minSup = minsup
        self._maxPer = maxPer
        self._sep = sep
        ...
        for i in range(0, len(candidates)):
            ...
            for j in range(i + 1, len(candidates)):
                ...
                if i1_list[:-1] == i2_list[:-1]:
                    union_DiffSet = self._finalPatterns[item2][2].union(
                        self._finalPatterns[item1][2])
                    ...
                    if union_supp >= self._minSup and period <= self._maxPer:
                        ...
                        self._finalPatterns[newKey] = [
                            union_supp, period, union_DiffSet]
...
def prepare_pfpm_data(raw_data:str):
    s = raw_data.replace('\t', '')
    s = s.replace('\n', '')
    return s
